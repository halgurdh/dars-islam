...
import { SALAH_STEPS, BuilderItem } from '../data/salah';
...
export class BuilderScene extends Phaser.Scene {
  ...
  private pickItems(): BuilderItem[] {
    const byDifficulty = [...SALAH_STEPS].sort((a, b) => this.difficultyOf(a) - this.difficultyOf(b));
  ...
  private buildTray(): void {
    ...
    const otherArabic = SALAH_STEPS.filter((h) => h.id !== this.currentItem.id).map((h) => h.arabic);
  ...
  private showComplete(): void {
    ...
    this.add.text(
      ...
      t().roundSummary(this.roundItems.length, this.mistakes, `${mm}:${ss}`, progress.count(), SALAH_STEPS.length),
      {
        fontFamily: LATIN_FONT,
        fontSize: '15px',
        color: COLORS.textMuted,
