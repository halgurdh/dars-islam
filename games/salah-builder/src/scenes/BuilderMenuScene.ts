...
interface DifficultyOption {
  ...
  // Hard sees everything including the longest. 1.0 = no cap. The 9 step
  // names here are all a similar length, so this alone doesn't move the
  // needle much — see distractorRange below.
  ...
  // Medium/Hard feel different here.
...
export class BuilderMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, height * 0.12, 'الصَّلاَة', {
    ...
    this.add.text(width / 2, height * 0.27, t().itemsLearned(learned, SALAH_STEPS.length), {
      fontFamily: LATIN_FONT,
      fontSize: '15px',
      color: hex(COLORS.accent),
    }).setOrigin(0.5);
