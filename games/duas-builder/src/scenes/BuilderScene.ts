...
import { DUAS, BuilderItem } from '../data/phrases';
...
export class BuilderScene extends Phaser.Scene {
  ...
  // (Easy never sees the longest phrases), and — separately — lets a
  // chosen round itself ramp from its easiest item to its hardest rather
  // than hitting them in a random order.
  ...
  // to [1] on its last, so a round gets visibly harder to build (more
  // decoy words to sift through) as it goes, not just longer.
  ...
  private pickItems(): BuilderItem[] {
    const byDifficulty = [...DUAS].sort((a, b) => this.difficultyOf(a) - this.difficultyOf(b));
  ...
  private buildTray(): void {
    ...
    const otherArabic = DUAS.filter((h) => h.id !== this.currentItem.id).map((h) => h.arabic);
  ...
  private showComplete(): void {
    ...
    this.add.text(
      ...
      t().roundSummary(this.roundItems.length, this.mistakes, `${mm}:${ss}`, progress.count(), DUAS.length),
      {
        fontFamily: LATIN_FONT,
        fontSize: '15px',
        color: COLORS.textMuted,
