...
import { PILLARS, BuilderItem } from '../data/pillars';
...
export class BuilderScene extends Phaser.Scene {
  ...
  private pickItems(): BuilderItem[] {
    const byDifficulty = [...PILLARS].sort((a, b) => this.difficultyOf(a) - this.difficultyOf(b));
  ...
  private buildTray(): void {
    ...
    const otherArabic = PILLARS.filter((h) => h.id !== this.currentItem.id).map((h) => h.arabic);
  ...
  private showComplete(): void {
    ...
    this.add.text(
      ...
      t().roundSummary(this.roundItems.length, this.mistakes, `${mm}:${ss}`, progress.count(), PILLARS.length),
      {
        fontFamily: LATIN_FONT,
        fontSize: '15px',
        color: COLORS.textMuted,
