...
interface DifficultyOption {
  ...
  // Hard sees everything including the longest. 1.0 = no cap. All 5 pillar
  // names are the same length here, so this alone doesn't move the needle
  // — see distractorRange below.
  maxDifficultyPercentile: number;
  // [min, max] decoy tiles in the tray, ramping from min on the round's
  // first item to max on its last — this is what actually makes Easy/
  // Medium/Hard feel different here, since every pillar name is a similar
  // length so item-length gating alone can't.
...
export class BuilderMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, height * 0.12, 'أَرْكَانُ الْإِسْلَام', {
    ...
    this.add.text(width / 2, height * 0.27, t().itemsLearned(learned, PILLARS.length), {
      fontFamily: LATIN_FONT,
      fontSize: '15px',
      color: hex(COLORS.accent),
    }).setOrigin(0.5);
