...
interface DifficultyOption {
  ...
  // 5 pillars every round, so this stays at 1 — see distractorRange below
...
export class BuilderMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, height * 0.1, 'أَرْكَانُ الْإِسْلَام', {
    ...
    this.add.text(width / 2, height * 0.232, t().itemsLearned(learned, PILLARS.length), {
      fontFamily: LATIN_FONT,
      fontSize: '14px',
      color: hex(COLORS.accent),
    }).setOrigin(0.5);
