...
export class GameScene extends Phaser.Scene {
  ...
  private pickNames(): SurahEntry[] {
    const unlearned = JUZ_AMMA_SURAHS.filter((n) => !progress.has(n.id));
    const learned = JUZ_AMMA_SURAHS.filter((n) => progress.has(n.id));
  ...
  private showComplete(): void {
    ...
    this.add.text(
      ...
      t().roundSummary(this.pairs, this.moves, `${mm}:${ss}`, progress.count(), JUZ_AMMA_SURAHS.length),
      {
        fontFamily: LATIN_FONT,
        fontSize: '15px',
        color: COLORS.textMuted,
