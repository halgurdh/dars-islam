...
import { PROPHETS, BuilderItem } from '../data/prophets';
...
export class BuilderScene extends Phaser.Scene {
  ...
  private pickItems(): BuilderItem[] {
    const byDifficulty = [...PROPHETS].sort((a, b) => this.difficultyOf(a) - this.difficultyOf(b));
  ...
  private buildTray(): void {
    ...
    const otherArabic = PROPHETS.filter((h) => h.id !== this.currentItem.id).map((h) => h.arabic);
  ...
  private showComplete(): void {
    ...
    this.add.text(
      ...
      t().roundSummary(this.roundItems.length, this.mistakes, `${mm}:${ss}`, progress.count(), PROPHETS.length),
      {
        fontFamily: LATIN_FONT,
        fontSize: '15px',
        color: COLORS.textMuted,
