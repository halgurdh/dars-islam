...
interface DifficultyOption {
  ...
  // Hard sees everything including the longest. 1.0 = no cap. Prophets'
  // names vary a lot in length (Hud/Lut vs Sulaiman/Ibrahim), so this
  // meaningfully changes what each tier draws from.
...
const DIFFICULTIES: DifficultyOption[] = [
  { label: () => t().easy, itemsPerRound: 8, maxDifficultyPercentile: 0.4, distractorRange: [1, 2] },
  { label: () => t().medium, itemsPerRound: 15, maxDifficultyPercentile: 0.75, distractorRange: [2, 4] },
  { label: () => t().hard, itemsPerRound: 25, maxDifficultyPercentile: 1, distractorRange: [3, 6] },
];
...
export class BuilderMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, height * 0.1, 'الْأَنْبِيَاء', {
    ...
    this.add.text(width / 2, height * 0.232, t().itemsLearned(learned, PROPHETS.length), {
      fontFamily: LATIN_FONT,
      fontSize: '14px',
      color: hex(COLORS.accent),
    }).setOrigin(0.5);
