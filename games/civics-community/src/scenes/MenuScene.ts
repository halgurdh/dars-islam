...
const GAME_ID = 'civics-community';
...
function generateQuizQuestion(): QuizQuestion {
  ...
  const meanings = [correct, ...distractors].map((t2) => meaningFor(t2));
  const choices = shuffle(meanings);
  return {
...
function generateSequenceRound(count: number): () => SequenceItem[] {
  return () => {
    const pool = shuffle(TERMS).slice(0, count) as CivicsTerm[];
    const sorted = [...pool].sort((a, b) => a.scope - b.scope);
