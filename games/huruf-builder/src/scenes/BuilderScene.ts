...
import { HURUF, BuilderItem } from '../data/huruf';
...
export class BuilderScene extends Phaser.Scene {
  ...
  private pickItems(): BuilderItem[] {
    const byDifficulty = [...HURUF].sort((a, b) => this.difficultyOf(a) - this.difficultyOf(b));
  ...
  private buildTray(): void {
    ...
    const otherArabic = HURUF.filter((h) => h.id !== this.currentItem.id).map((h) => h.arabic);
  ...
  private showComplete(): void {
    ...
    this.add.text(
      ...
      t().roundSummary(this.roundItems.length, this.mistakes, `${mm}:${ss}`, progress.count(), HURUF.length),
      {
        fontFamily: LATIN_FONT,
        fontSize: '15px',
        color: COLORS.textMuted,
