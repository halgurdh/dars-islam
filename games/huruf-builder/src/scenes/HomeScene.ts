...
import { fillBlankQuestion, trueFalseStatement, toTrueFalseQuestion } from '@shared/quiz-variants';
...
function generateListenQuestion(): QuizQuestion {
  const [correct, ...distractors] = shuffle(HURUF).slice(0, 4);
  const choices = shuffle([correct, ...distractors].map((h) => meaningFor(h)));
  return {
    prompt: '',
    choices,
