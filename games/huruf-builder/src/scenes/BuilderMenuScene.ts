...
interface DifficultyOption {
  ...
  // tier is allowed to draw from — Easy only ever sees the simplest items,
  // Hard sees everything including the longest. 1.0 = no cap. Matters most
  // for games whose items actually vary in length (prophets, phrases, ...);
  // huruf-builder's letter names are all the same length, so this alone
  // doesn't move the needle here — see distractorRange below.
  maxDifficultyPercentile: number;
  // [min, max] decoy tiles in the tray, ramping from min on the round's
  // first item to max on its last — this is what actually makes huruf-
  // builder's Easy/Medium/Hard feel different, since every letter name is
  // the same length (always 3 pieces) so item-length gating alone can't.
  distractorRange: [number, number];
}

const DIFFICULTIES: DifficultyOption[] = [
  { label: () => t().easy, itemsPerRound: 6, maxDifficultyPercentile: 0.4, distractorRange: [1, 2] },
  { label: () => t().medium, itemsPerRound: 10, maxDifficultyPercentile: 0.75, distractorRange: [2, 4] },
