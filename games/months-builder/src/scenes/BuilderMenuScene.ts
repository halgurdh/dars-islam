...
interface DifficultyOption {
  ...
  // 12 months every round, so this stays at 1 — see distractorRange below
...
// Easy/Medium/Hard all drew all 12 months every round, differing only in
...
// 12 months in one session.
const DIFFICULTIES: DifficultyOption[] = [
  { label: () => t().easy, itemsPerRound: 6, maxDifficultyPercentile: 0.6, distractorRange: [1, 2] },
  { label: () => t().medium, itemsPerRound: 9, maxDifficultyPercentile: 0.8, distractorRange: [2, 4] },
...
export class BuilderMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, height * 0.1, 'الشُّهُور', {
    ...
    this.add.text(width / 2, height * 0.232, t().itemsLearned(learned, MONTHS.length), {
      fontFamily: LATIN_FONT,
      fontSize: '14px',
      color: hex(COLORS.accent),
    }).setOrigin(0.5);
