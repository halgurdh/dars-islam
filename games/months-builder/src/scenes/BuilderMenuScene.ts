...
interface DifficultyOption {
  ...
  // Hard sees everything including the longest. 1.0 = no cap. This game
  // always plays all 12 months in one round, so this stays at 1 — see
  // distractorRange below for what actually varies by tier.
...
const DIFFICULTIES: DifficultyOption[] = [
  { label: () => t().easy, itemsPerRound: 12, maxDifficultyPercentile: 1, distractorRange: [1, 2] },
  { label: () => t().medium, itemsPerRound: 12, maxDifficultyPercentile: 1, distractorRange: [2, 4] },
  { label: () => t().hard, itemsPerRound: 12, maxDifficultyPercentile: 1, distractorRange: [3, 6] },
];
...
export class BuilderMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, height * 0.12, 'الشُّهُور', {
    ...
    this.add.text(width / 2, height * 0.27, t().itemsLearned(learned, MONTHS.length), {
      fontFamily: LATIN_FONT,
      fontSize: '15px',
      color: hex(COLORS.accent),
    }).setOrigin(0.5);
